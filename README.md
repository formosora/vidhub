...
├── test/smoke.sh   # API 冒烟测试（安全回归 + 权限边界）
...
`test/smoke.sh`（81 条断言，不需要 ffmpeg）——注册开关与验证码、可见性与广场过滤、
双语 API、上传内容不可执行、最后一个管理员不可锁死、公开接口不泄露 IP、设置项越界
钳制、隔离内容不可重传、配额与防盗链、越权边界、Range/路径穿越。
